...
| Token behavior assumptions | Fee-on-transfer and rebasing assets can break staged accounting; callback-enabled assets expand the external-call surface. | Use standard non-rebasing ERC-20 assets. Deposit intake measures the received balance delta, and one shared reentrancy guard protects `requestDeposit`, `settleEpoch`, `withdraw`, and `redeem`. |
